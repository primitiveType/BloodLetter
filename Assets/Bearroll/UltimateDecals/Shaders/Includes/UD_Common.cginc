...
UNITY_DEFINE_INSTANCED_PROP(float, _MaskThreshold)
...
float ProcessHeightAndMask(float4 uv, float4 dd) {
	...
	float maskMin = UNITY_ACCESS_INSTANCED_PROP(Props, _MaskThreshold);
	float maskMax = saturate(maskMin + _MaskSmoothing);
