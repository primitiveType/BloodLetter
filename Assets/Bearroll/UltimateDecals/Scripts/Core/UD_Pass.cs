    ...
    public class UD_Pass
    {
        ...
        public void RebuildCommandBuffer(UD_Camera camera, CommandBuffer commandBuffer)
        {
            ...
            foreach (var batch in batches)
            {
                ...
                commandBuffer.DrawMeshInstanced(ResourceManager.instanceMesh, 0, batch.material, pass, batch.matrices,
                    batch.count, batch.props);
            }
        }
        ...
        public void AddDecal(Material material, Transform t, bool _, float maskThreshold,
            float normalizedAtlasOffset = 0)
        {
            GetBatchByMaterial(material).AddDecal(t, _, maskThreshold, normalizedAtlasOffset);
        }

        public void AddDecal(Material material, Matrix4x4 matrix, bool _, float maskThreshold,
            float normalizedAtlasOffset = 0)
        {
            GetBatchByMaterial(material).AddDecal(matrix, _, maskThreshold, normalizedAtlasOffset);
        }
