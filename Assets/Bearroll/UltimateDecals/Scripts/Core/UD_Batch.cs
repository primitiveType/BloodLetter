...
namespace Bearroll.UltimateDecals {
	...
	public class UD_Batch {
		...
		Shader shader;
		private static readonly int MaskThresholdProperty = Shader.PropertyToID("_MaskThreshold");
		private static readonly int ShArProperty = Shader.PropertyToID("_SHAr");
		private static readonly int ShAgProperty = Shader.PropertyToID("_SHAg");
		private static readonly int ShAbProperty = Shader.PropertyToID("_SHAb");
		private static readonly int ShBrProperty = Shader.PropertyToID("_SHBr");
		private static readonly int ShBgProperty = Shader.PropertyToID("_SHBg");
		private static readonly int ShBbProperty = Shader.PropertyToID("_SHBb");
		private static readonly int ShcProperty = Shader.PropertyToID("_SHC");
		...
		void UpdateProps() {
			...
			if (manager.perDecalLightProbes) {
				...
				props.SetFloatArray(MaskThresholdProperty, alphaCutoffs);
				props.SetVectorArray(ShArProperty, SHAr);
				props.SetVectorArray(ShAgProperty, SHAg);
				props.SetVectorArray(ShAbProperty, SHAb);
				props.SetVectorArray(ShBrProperty, SHBr);
				props.SetVectorArray(ShBgProperty, SHBg);
				props.SetVectorArray(ShBbProperty, SHBb);
				props.SetVectorArray(ShcProperty, SHC);
		...
		public int AddDecal(Matrix4x4 matrix, bool _, float maskThreshold , float normalizedAtlasOffset = 0) {
			...
			alphaCutoffs[index] = maskThreshold;
			var r = index;
		...
		public int AddDecal(Transform t, bool _, float maskThreshold, float normalizedAtlasOffset = 0) {
		...
		public int AddDecal(UltimateDecal decal) {
			...
			alphaCutoffs[index] = decal.alphaCutoff;
		...
		void RemoveDecal(int index) {
			...
			for (var j = index; j < count - 1; j++) {
				...
				alphaCutoffs[j] = alphaCutoffs[j + 1];
