...
public class EditorObjExporter : ScriptableObject
{
	...
	//the reader.
	private static string targetFolder = "Assets/LevelMeshes/test";
 
 
	private static string MeshToString(Mesh mf, Dictionary<string, ObjMaterial> materialList, Transform transform)
	{
	...
	private static Mesh MeshToFile(Mesh mf, string folder, string filename, Transform transform) 
	{
		Dictionary<string, ObjMaterial> materialList = PrepareFileWrite();

		string fullPath = folder + Path.DirectorySeparatorChar + filename + ".obj";
		using (StreamWriter sw = new StreamWriter(fullPath)) 
		{
			sw.Write("mtllib ./" + filename + ".mtl\n");
 
			sw.Write(MeshToString(mf, materialList, transform));
		}

		string path = fullPath;//FileUtil.GetProjectRelativePath(fullPath);
	...
	private static void MeshesToFile(MeshFilter[] mf, string folder, string filename) 
	{
		...
		using (StreamWriter sw = new StreamWriter(folder +Path.PathSeparator + filename + ".obj")) 
		{
			...
			{
				sw.Write(MeshToString(mf[i].sharedMesh, materialList, mf[i].transform));
			}
		}
	...
	private static HashSet<Transform> processedTransforms;
	...
	static void ExportSelectionToSeparate()
	{
		...
		processedTransforms = new HashSet<Transform>();
		...
		for (int i = 0; i < selection.Length; i++)
		{
			...
			{
				exportedObjects++;
				Transform transform = meshfilter[m].transform;
				var mesh = MeshToFile(meshfilter[m].sharedMesh, targetFolder, selection[i].name + "_" + i + "_" + m, transform);
				meshfilter[m].sharedMesh = mesh;
			...
			//some objs contain only a mesh collider.
			MeshCollider[] meshColliders = selection[i].GetComponentsInChildren<MeshCollider>();
 
			for (int m = 0; m < meshColliders.Length; m++)
			{
				Transform transform = meshColliders[m].transform;
				...
				exportedObjects++;
				var mesh = MeshToFile(meshColliders[m].sharedMesh, targetFolder, selection[i].name + "_" + i + "_" + m, transform);
				meshColliders[m].sharedMesh = mesh;
				processedTransforms.Add(transform);
			}
		}
